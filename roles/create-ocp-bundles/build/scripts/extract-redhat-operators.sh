...
log -n "Creating OCP ImageSet $DESTDIR/operators/redhat"
mkdir -p $DESTDIR/operators/redhat
if [ $? == 0 ]; then
  log "Creating OCP Binaries $DESTDIR/operators/redhat ... done"
else
  log "Creating OCP Binaries $DESTDIR/operators/redhat ... failed"
  log "Ensure you have the proper permissions to create $DESTDIR/operators/redhat"
  exit 2
fi
...
do
  mv $filename $DESTDIR/operators/redhat &&
  echo "OCP Red Hat operator [$(basename $filename)] extracted successfully!"
done
