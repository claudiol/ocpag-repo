...
log -n "Creating OCP ImageSet $DESTDIR/ocp-release ... "
mkdir -p $DESTDIR/ocp-release
if [ $? == 0 ]; then
  log "Creating OCP Binaries $DESTDIR/ocp-release ... done"
else
  log "Creating OCP Binaries $DESTDIR/ocp-release ... failed"
  log "Ensure you have the proper permissions to create $DESTDIR/ocp-release"
  exit 2
fi
...
do
  #tar -xvf $filename -C "$DESTDIR/ocp-release" &&
  mv $filename $DESTDIR/ocp-release &&
