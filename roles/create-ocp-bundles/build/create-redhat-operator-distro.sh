...
cp $ROLEDIR/build/scripts/extract-redhat-operators.sh $TEMPDIR/image_stage/
for i in `ls $BUNDLEDIR/operators/redhat/*.tar`
do 
  BASE=$(basename $i)
  cp $i $TEMPDIR/image_stage/$BASE
  makeself --sha256 $TEMPDIR/image_stage  $TEMPDIR/ocp-redhat-operators-installer-$COUNT.run "OpenShift Image Set Installer" ./extract-redhat-operators.sh
