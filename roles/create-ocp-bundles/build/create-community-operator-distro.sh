...
cp $ROLEDIR/build/scripts/extract-community-operators.sh $TEMPDIR/image_stage/
for i in `ls $BUNDLEDIR/operators/community/*.tar`
do 
  BASE=$(basename $i)
  cp $i $TEMPDIR/image_stage/$BASE
  makeself --sha256 $TEMPDIR/image_stage  $TEMPDIR/ocp-community-operators-installer-$COUNT.run "OpenShift Image Set Installer" ./extract-community-operators.sh
